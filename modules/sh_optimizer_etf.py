...
def max_sf(mean_returns, cov_matrix, bench_std, riskfreerate = .03, constraint_set=(.01,.60)):
    ...
    if 1==2:
        ...
        constraints = ({'type': 'eq', 'fun': lambda x: np.sum(x) - 1},
                       {'type': 'eq', 'fun': lambda x: (np.sqrt( np.dot(pd.DataFrame(x).T.values.tolist(), np.dot(cov_matrix, x)) ) * np.sqrt(252))-bench_std})
